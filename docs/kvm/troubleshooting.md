...
## Resize and Expand Disk

### In-Guest Resize Flow
...
### Offline Resize with `qemu-nbd`

```bash
sudo qemu-img resize /var/lib/libvirt/images/debian11.qcow2 100G

sudo modprobe nbd max_part=8
sudo qemu-nbd -c /dev/nbd0 /var/lib/libvirt/images/debian11.qcow2  # connect to NBD device
...
### Partition Cleanup Notes

```bash
swapoff -a
# umount related partitions, use fdisk -l /dev/vda
umount /dev/vda5 2>/dev/null
# delete some partitions
```

## VM Process and `virsh` Stuck Issues
...
### EFI Shell and Cross-Host Repair
