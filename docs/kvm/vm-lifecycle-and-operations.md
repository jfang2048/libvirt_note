...
for vm in $(sudo virsh list --all --name); do
    echo "$vm"
    sudo virsh dumpxml "$vm" | grep -i "<source file" | sed -E "s/.*file='([^']+)'.*/\1/"
    echo "-------------------"
done
```
...
```bash
# In libvirt 10.9.0 both internal and external snapshots work with UEFI guests,
# as long as the guest uses qcow2-backed NVRAM.
# If a VM was created over 2 years ago, it may use raw-backed NVRAM and need reconfiguration.
